...
# -----------------------------
# SAVE FINAL MODEL (WORKING FIX)
# -----------------------------
import tensorflow as tf
os.makedirs("Model", exist_ok=True)

# 🔥 Remove optimizer state (important fix)
model.compile(
    optimizer="adam",
    loss="binary_crossentropy",
    metrics=["accuracy"]
...
try:
    # ✅ Save only architecture + weights (no optimizer)
    model.save("Model/final_model.h5", include_optimizer=False)
    print("✅ Full model saved as final_model.h5")
